...
class GraphBuilder:
    ...
    def ai_news_builder_graph(self):
        ...
        # Add edges
        self.graph_builder.add_edge(START, "fetch_news") # Vừa thêm mới
...
class RAGGraphBuilder(GraphBuilder):
    def __init__(self,model, vectorstore_path, embedding_model):
        ...
        self.rag_chatbot_node = None  # Initialize as None
    
    def update_vectorstore_path(self, new_vectorstore_path):
        """Update vectorstore path and refresh chatbot node if needed"""
        if new_vectorstore_path != self.vectorstore_path:
            self.vectorstore_path = new_vectorstore_path
            if self.rag_chatbot_node:
                self.rag_chatbot_node.update_vectorstore_path(new_vectorstore_path)
